...
@Component({
  ...
  templateUrl: './myreply.component.html',
  styleUrls: ['./myreply.component.scss'],
})
export class MyreplyComponent implements OnInit {
    ...
    this.http.getUser().then((response:any) => {
      if(response){
        if(response.guid){
          ...
          const params = { exchange_proceeding:this.exchange_proceeding,publish_status_code: '02',sort:'-publish_time',comment_person_id:this.person_id };
          this.getData(params);
        }
      }
    })
